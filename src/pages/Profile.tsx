  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setEditMode(false);
        setQuizzes(prev => prev.map(q => q.id === selectedQuiz.id ? { ...editing, id: selectedQuiz.id } : q));
      } else {
        const err = await res.json().catch(() => ({}));
        setSnackbar({ msg: `Save failed: ${err.error || res.statusText}`, severity: 'error' });
      }
    } catch {
  ...
  const addQuestion = () => {
    ...
    items.push({
      ...
      question: {
        ...
        hint: '',
        explanation: '',
        type: { name: 'multiple_choice' as const, description: 'Multiple choice question', correctAnswer: '', incorrectAnswers: [], options: [] },
        category: [],
        correctAnswers: [],
        incorrectAnswers: [],
        options: [],
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', minHeight: 'calc(100vh - 80px)', px: 2, py: 4, gap: 4 }}>
      ...
      >
        ...
        <CardContent sx={{ p: 4, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 2 }}>
          <Avatar
            sx={{ width: 80, height: 80, bgcolor: theme.palette.primary.main, fontSize: '2rem', fontWeight: 700, mb: 1 }}
          >
            {email.charAt(0).toUpperCase()}
          </Avatar>
          <Typography variant="h5" sx={{ fontWeight: 700 }}>{name || 'User'}</Typography>
          ...
          <Button
            ...
            sx={{ mt: 2, borderRadius: 2, fontWeight: 600, textTransform: 'none', py: 1.2, borderColor: theme.palette.error.main, color: theme.palette.error.main, '&:hover': { borderColor: theme.palette.error.dark, bgcolor: 'rgba(244,67,54,0.04)' } }}
          >
            Sign out
          </Button>
        </CardContent>
        ...
        {selectedQuiz ? (
          <>
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 2 }}>
              <IconButton onClick={closeDetail} size="small"><ArrowBackIcon /></IconButton>
            ...
            {quizLoading ? (
              ...
              <Paper sx={{ p: 3, borderRadius: 2 }}>
                {/* Quiz Metadata */}
                <TextField label="Quiz Name" fullWidth size="small" sx={{ mb: 2 }}
                ...
                <TextField label="Description" fullWidth size="small" multiline rows={2} sx={{ mb: 2 }}
                  ...
                  onChange={e => updateQuizField('quizDescription', e.target.value)} />
                  ...
                  <Card key={i} variant="outlined" sx={{ mb: 1.5, borderRadius: 2 }}>
                    <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 1 }}>
                        <Chip
                          icon={item.kind === 'slide' ? <SlideshowIcon /> : <HelpIcon />}
                          label={`${item.kind} #${i + 1}`}
                          size="small"
                      ...
                      {item.kind === 'slide' ? (
                        <>
                          <TextField label="Title" fullWidth size="small" sx={{ mb: 1 }}
                            value={item.slide?.title || ''}
                            onChange={e => updateItem(i, 'title', e.target.value)} />
                          <TextField label="Content" fullWidth size="small" multiline rows={2}
                            value={item.slide?.content || ''}
                            onChange={e => updateItem(i, 'content', e.target.value)} />
                        </>
                      ) : (
                        <>
                          <TextField label="Question" fullWidth size="small" sx={{ mb: 1 }}
                            value={item.question?.question || ''}
                            onChange={e => updateItem(i, 'question', e.target.value)} />
                          <Box sx={{ display: 'flex', gap: 1, mb: 1 }}>
                            <TextField label="Points" type="number" size="small" sx={{ width: 100 }}
                              value={item.question?.points || 0}
                              onChange={e => updateItem(i, 'points', parseInt(e.target.value) || 0)} />
                            <TextField label="Difficulty (1-5)" type="number" size="small" sx={{ width: 140 }}
                              value={item.question?.difficulty || 1}
                              inputProps={{ min: 1, max: 5 }}
                              onChange={e => updateItem(i, 'difficulty', parseInt(e.target.value) || 1)} />
                          </Box>
                          <TextField label="Hint" fullWidth size="small" sx={{ mb: 1 }}
                            value={item.question?.hint || ''}
                            onChange={e => updateItem(i, 'hint', e.target.value)} />
                          <TextField label="Explanation" fullWidth size="small" sx={{ mb: 1 }} multiline rows={2}
                            value={item.question?.explanation || ''}
                            onChange={e => updateItem(i, 'explanation', e.target.value)} />
                          <Box sx={{ display: 'flex', gap: 1, flexDirection: { xs: 'column', sm: 'row' } }}>
                            <TextField label="Options (comma-separated)" fullWidth size="small"
                              value={(item.question?.options || []).join(', ')}
                              onChange={e => updateItem(i, 'options', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} />
                            <TextField label="Correct Answers (comma-sep)" fullWidth size="small"
                              value={(item.question?.correctAnswers || []).join(', ')}
                              onChange={e => updateItem(i, 'correctAnswers', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} />
                          </Box>
                          <Box sx={{ display: 'flex', gap: 1, mt: 1, flexDirection: { xs: 'column', sm: 'row' } }}>
                            <TextField label="Incorrect Answers (comma-sep)" fullWidth size="small"
                              value={(item.question?.incorrectAnswers || []).join(', ')}
                              onChange={e => updateItem(i, 'incorrectAnswers', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} />
                            <TextField label="Categories (comma-sep)" fullWidth size="small"
                              value={(item.question?.category || []).join(', ')}
                              onChange={e => updateItem(i, 'category', e.target.value.split(',').map(s => s.trim()).filter(Boolean))} />
                          </Box>
                        </>
                      )}
                    </CardContent>
                ...
                <Box sx={{ display: 'flex', gap: 1, mt: 1 }}>
                  <Button variant="outlined" size="small" startIcon={<SlideshowIcon />} onClick={addSlide}>
                    Add Slide
                  </Button>
                  <Button variant="outlined" size="small" startIcon={<HelpIcon />} onClick={addQuestion}>
                    Add Question
                  </Button>
                </Box>
                ...
                <Box sx={{ display: 'flex', gap: 1 }}>
                  ...
                  <Button variant="outlined" onClick={handleCancel} disabled={saving} sx={{ borderRadius: 2, textTransform: 'none' }}>
                    Cancel
                  </Button>
                </Box>
              </Paper>
              ...
              <Paper sx={{ p: 3, borderRadius: 2 }}>
                ...
                <Box sx={{ display: 'flex', gap: 2, mb: 2, flexWrap: 'wrap' }}>
                  ...
                  <Typography variant="caption" color="text.secondary">
                    Items: <strong>{selectedQuiz.quizItems?.length || 0}</strong>
                  </Typography>
                  <Typography variant="caption" color="text.secondary">
                    Questions: <strong>{selectedQuiz.quizItems?.filter(i => i.kind === 'question').length || 0}</strong>
                  </Typography>
                </Box>
                <Divider sx={{ mb: 2 }} />
                <Typography variant="subtitle1" sx={{ fontWeight: 600, mb: 1 }}>Items</Typography>
                {(!selectedQuiz.quizItems || selectedQuiz.quizItems.length === 0) ? (
                  <Typography variant="body2" color="text.secondary">No items in this quiz.</Typography>
                ) : (
                  selectedQuiz.quizItems.map((item, i) => (
                    <Card key={i} variant="outlined" sx={{ mb: 1, borderRadius: 2 }}>
                      <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                        ...
                        {item.kind === 'slide' ? (
                          <>
                            {item.slide?.title && <Typography variant="body2" sx={{ fontWeight: 600 }}>{item.slide.title}</Typography>}
                            {item.slide?.content && <Typography variant="body2" color="text.secondary">{item.slide.content}</Typography>}
                          </>
                        ) : item.question ? (
                          <>
                            <Typography variant="body2" sx={{ fontWeight: 500 }}>{item.question.question}</Typography>
                            <Box sx={{ display: 'flex', gap: 2, mt: 0.5, flexWrap: 'wrap' }}>
                              <Typography variant="caption" color="text.secondary">{item.question.points} pts</Typography>
                              <Typography variant="caption" color="text.secondary">Difficulty: {item.question.difficulty}</Typography>
                              {item.question.options && item.question.options.length > 0 && (
                                <Typography variant="caption" color="text.secondary">{item.question.options.length} options</Typography>
                              )}
                              {item.question.category && item.question.category.length > 0 && (
                                <Typography variant="caption" color="text.secondary">
                                  Categories: {item.question.category.join(', ')}
                                </Typography>
                              )}
                            </Box>
                            {item.question.options && item.question.options.length > 0 && (
                              <Box sx={{ mt: 0.5, display: 'flex', flexWrap: 'wrap', gap: 0.5 }}>
                                {item.question.options.map((opt, oi) => (
                                  <Chip key={oi} label={opt} size="small" variant="outlined" sx={{ height: 22, fontSize: '0.75rem' }} />
                                ))}
                              </Box>
                            )}
                          </>
          ...
          <>
            <Typography variant="h6" sx={{ fontWeight: 700, mb: 2 }}>My Quizzes</Typography>
            {quizzesLoading ? (
              <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}><CircularProgress size={32} /></Box>
            ) : quizzes.length === 0 ? (
              <Typography variant="body2" color="text.secondary" sx={{ textAlign: 'center', py: 4 }}>
                No quizzes yet.{' '}
                <Button variant="text" size="small" onClick={() => navigate('/create-quiz')} sx={{ textTransform: 'none' }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.5 }}>
                {quizzes.map((quiz, index) => (
                  <Card
                    key={quiz.id ?? index}
                    variant="outlined"
                    sx={{ borderRadius: 2, cursor: 'pointer', '&:hover': { borderColor: theme.palette.primary.main } }}
                    onClick={() => quiz.id && openQuiz(quiz.id)}
                  >
                    <CardContent sx={{ p: 2, '&:last-child': { pb: 2 } }}>
                      <Typography variant="subtitle1" sx={{ fontWeight: 600 }}>{quiz.quizName}</Typography>
                      {quiz.quizDescription && (
                        <Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>{quiz.quizDescription}</Typography>
                      )}
                      <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
                        {quiz.quizItems.filter(i => i.kind === 'question').length} question{quiz.quizItems.filter(i => i.kind === 'question').length !== 1 ? 's' : ''}
                        {' · '}
                        {quiz.quizItems.length} item{quiz.quizItems.length !== 1 ? 's' : ''}
                      </Typography>
                    </CardContent>
                  </Card>
                ))}
      ...
      <Dialog open={deleteOpen} onClose={() => setDeleteOpen(false)}>
        ...
        <DialogContent>
          <DialogContentText>
            Are you sure you want to delete "{selectedQuiz?.quizName}"? This cannot be undone.
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={() => setDeleteOpen(false)} sx={{ textTransform: 'none' }}>Cancel</Button>
          <Button onClick={handleDelete} color="error" variant="contained" disabled={saving} sx={{ textTransform: 'none' }}>
