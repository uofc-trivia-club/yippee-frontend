...
export default function HostGame() {
  ...
  const handleHostGame = async () => {
    ...
    console.log(
      "sending the request of createLobby with: ",
      selectedQuiz,
      user,
    );
      ...
      >
        ...
        {!isAuthenticated && (
          <Alert severity="info" sx={{ mb: 2 }}>
            ...
            <Link href="/sign-in" sx={{ fontWeight: 600 }}>
              Sign in
            </Link>{" "}
            to host all quizzes and create your own.
          </Alert>
        )}
        ...
        <TextField
          id="host-name"
          label={
            isAuthenticated ? "Display Name (optional)" : "Enter Your Name"
          }
          variant="outlined"
          fullWidth
          value={hostName}
          ...
          helperText={
            isAuthenticated
              ? "Will default to your account name if left empty"
              : `${hostName.length}/20 characters`
          }
          sx={{ marginBottom: 2 }}
        />
        <SelectQuiz
        ...
        {selectedQuiz && (
          <Typography
            variant="h6"
            gutterBottom
            className={styles.selectQuizTitle}
          >
