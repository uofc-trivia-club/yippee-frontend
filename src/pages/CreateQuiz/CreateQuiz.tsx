...
function SortableQuestionCard({
  ...
  onToggle: () => void;
  onDelete: () => void;
  onChange: <K extends keyof QuizQuestionForm>(
    field: K,
  ...
  const hasOptionContent = (option: QuizQuestionForm["options"][number]) =>
    ...
    Boolean(option.imageFile) ||
    Boolean((option.imageUrl || "").trim()) ||
    Boolean((option.imageId || "").trim());
  const isValid = isFreeTextType
    ? question.question.trim() && question.acceptedAnswers.length > 0
    : isCalendarType
      ? question.question.trim() &&
        question.acceptedAnswers.length > 0 &&
        question.acceptedAnswers.every((date) =>
          /^\d{4}-\d{2}-\d{2}$/.test((date || "").trim()),
        )
      : isNumericalType
        ? question.question.trim() &&
          Number.isFinite(Number((question.acceptedAnswers[0] || "").trim()))
        : isFillInBlankType
          ...
          : isPhraseMatchType
            ...
            : isPairType
              ...
              : question.question.trim() &&
                (allowsNoCorrect || correctAnswersCount > 0) &&
                question.options.every(hasOptionContent);
  const isEssayType = question.type === "essay";
  const isOrderType =
    question.type === "ranking" || question.type === "ordering";
  const isSingleCorrectType =
    question.type === "multiple" ||
    question.type === "dropdown" ||
    question.type === "true_false";
  const usesOptionsEditor =
    !isFreeTextType &&
    !isCalendarType &&
    !isFillInBlankType &&
    !isNumericalType &&
  ...
  const [questionCursorPos, setQuestionCursorPos] = useState<number | null>(
    null,
  );
  const [draggedPhraseBlankIndex, setDraggedPhraseBlankIndex] = useState<
    number | null
  >(null);
  ...
  const handleOptionImageDrop = (
    ...
    const imageFile =
      Array.from(fileList).find((file) => file.type.startsWith("image/")) ||
      null;
    if (!imageFile) return;
  ...
  const handlePhraseAnswerChange = (blankIndex: number, value: string) => {
    ...
    const syncedWordBank = nextAnswers
      .slice(0, phraseBlankCount)
      .map((answer, index) => ({
  ...
  const addFillBlank = () => {
    const separator =
      question.question.trim().length > 0 && !question.question.endsWith(" ")
        ? " "
        : "";
  ...
  const removeFillBlankAt = (blankIndex: number) => {
    ...
    nextParts[blankIndex] =
      `${nextParts[blankIndex]}${nextParts[blankIndex + 1]}`;
    nextParts.splice(blankIndex + 1, 1);
    const nextQuestionText = nextParts
      .join("____")
      .replace(/\s{2,}/g, " ")
      .trimEnd();

    const nextAnswers = question.acceptedAnswers.filter(
    ...
    >
      <CardContent sx={{ p: 2, "&:last-child": { pb: 2 } }}>
        ...
        <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
          ...
          {/* Options Count */}
          {isFreeTextType ? (
            <Chip label="Text answer" size="small" variant="outlined" />
          ) : isPairType ? (
            <Chip
          ...
          <Box sx={{ mt: 2, display: "flex", flexDirection: "column", gap: 2 }}>
                ...
                >
                  <Box>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      Add blanks in the sentence, then fill in the correct
                      answer for each one.
                    </Typography>
                  </Box>
                  <Button
            ...
            <Paper variant="outlined" sx={{ p: 2.5 }}>
              ...
              <Box sx={{ mb: 2 }}>
                ...
                >
                  Add one or more accepted answers. Players can submit any of
                  these.
                </Typography>
                ...
                >
                  <TextField
                    ...
                    onChange={(e) =>
                      onChange("acceptedAnswerInput", e.target.value)
                    }
                    onKeyDown={(e) => {
                      if (e.key === "Enter") {
                        e.preventDefault();
            ...
            {isNumericalType && (
              <Box sx={{ mb: 2 }}>
                ...
                <TextField
                  ...
                  onChange={(e) =>
                    onChange("acceptedAnswers", [e.target.value])
                  }
            ...
            {isCalendarType && (
              <Box sx={{ mb: 2 }}>
                ...
                <Typography
                  ...
                  sx={{ display: "block", mb: 1.5 }}
                >
                  Pick one or more correct dates using the calendar picker
                  (YYYY-MM-DD).
                </Typography>
                ...
                >
                  ...
                  <Button
                    ...
                    disabled={
                      !/^\d{4}-\d{2}-\d{2}$/.test(
                        (question.acceptedAnswerInput || "").trim(),
                      )
                    }
                  >
                    Add Date
                  ...
                  (isPairType ? (
                    ...
                    >
                      <Typography
                        ...
                        sx={{ mb: 2 }}
                      >
                        Use the left column for items on the left side and the
                        right column for their matches.
                      </Typography>
                          ...
                          <Box sx={{ display: "grid", gap: 0.75 }}>
                            ...
                            {pair.leftImageFile || pair.leftImageUrl ? (
                              <Box
                                ...
                                src={
                                  pair.leftImageFile
                                    ? URL.createObjectURL(pair.leftImageFile)
                                    : pair.leftImageUrl || ""
                                }
                          ...
                          <Box sx={{ display: "grid", gap: 0.75 }}>
                            ...
                            {pair.rightImageFile || pair.rightImageUrl ? (
                              <Box
                                ...
                                src={
                                  pair.rightImageFile
                                    ? URL.createObjectURL(pair.rightImageFile)
                                    : pair.rightImageUrl || ""
                                }
                      ...
                      {question.options.map((option, optionIndex) =>
                        (() => {
                          const shouldHighlightCorrect =
                            showCorrectSelector && option.isCorrect;
                          return (
                            <Box
                              key={optionIndex}
                              onDragOver={(event) => {
...
export default function CreateQuiz() {
    ...
    return timelineItems
      .map((itemRef, timelineIndex) => {
        ...
        const questionIndex = questions.findIndex(
          (question) => question.id === itemRef.refId,
        );
  ...
  const handleQuestionChange = <K extends keyof QuizQuestionForm>(
    ...
    if (field === "type") {
      ...
      if (
        ...
        !currentQuestionText.match(/_{3,}/)
      ) {
        updatedQuestions[globalIndex].question =
      ...
      if (selectedType === "multiple") {
        ...
        updatedQuestions[globalIndex].options = updatedQuestions[
          globalIndex
        ].options.map((opt, idx) => ({
          ...opt,
          isCorrect:
            firstCorrectIndex >= 0 ? idx === firstCorrectIndex : idx === 0,
        }));
      }
    }
  ...
  const handleOptionChange = (
    ...
    if (
      field === "isCorrect" &&
      value === true &&
      (qType === "multiple" || qType === "dropdown" || qType === "true_false")
    ) {
      updatedQuestions[globalIndex].options = updatedOptions.map(
        (opt, idx) => ({
          ...opt,
    ...
    if (field === "isCorrect" && value === false) {
      ...
      const currentCorrectCount = updatedOptions.filter(
        (opt) => opt.isCorrect,
      ).length;
  ...
  const addSlide = () => {
    ...
    setTimelineItems((prev) => [
      ...prev,
      { id: timelineId, kind: "slide", refId: newSlide.id },
    ]);
  ...
  const addQuestion = () => {
    ...
    setTimelineItems((prev) => [
      ...prev,
      { id: timelineId, kind: "question", refId: newQuestion.id },
    ]);
    setSelectedTimelineId(timelineId);
  ...
  const deleteSlide = (slideId: string) => {
    ...
    setTimelineItems((prev) =>
      prev.filter((item) => !(item.kind === "slide" && item.refId === slideId)),
    );
  };

  const addOption = (globalIndex: number) => {
    const updatedQuestions = [...questions];
    updatedQuestions[globalIndex].options.push({
      text: "",
      isCorrect: false,
      imageUrl: "",
      imageId: "",
  ...
  const removeMatchingPair = (globalIndex: number, pairIndex: number) => {
    ...
    const updatedQuestions = [...questions];
    updatedQuestions[globalIndex].matchingPairs = updatedQuestions[
      globalIndex
    ].matchingPairs.filter((_, index) => index !== pairIndex);
    setQuestions(updatedQuestions);
  };
  ...
  const removeAcceptedAnswer = (globalIndex: number, answerIndex: number) => {
    const updatedQuestions = [...questions];
    updatedQuestions[globalIndex].acceptedAnswers = updatedQuestions[
      globalIndex
    ].acceptedAnswers.filter((_, idx) => idx !== answerIndex);
    setQuestions(updatedQuestions);
  };
  ...
  const deleteQuestion = (globalIndex: number) => {
    ...
    setTimelineItems((prev) =>
      prev.filter(
        (item) => !(item.kind === "question" && item.refId === questionId),
      ),
    );
  ...
  const transformQuestionForSubmission = (
    question: QuizQuestionForm,
  ): QuizQuestion => {
    const typeName =
      question.type === "multiple" ? "multiple_choice" : question.type;
    ...
    const getOptionLabel = (
      opt: QuizQuestionForm["options"][number],
      index: number,
    ) => {
      ...
      const hasImage =
        Boolean(opt.imageFile) ||
        Boolean((opt.imageUrl || "").trim()) ||
        Boolean((opt.imageId || "").trim());
    ...
    const correctAnswers = optionEntries
      .filter((entry) => entry.opt.isCorrect)
      .map((entry) => entry.label);
    const incorrectAnswers = optionEntries
      .filter((entry) => !entry.opt.isCorrect)
      .map((entry) => entry.label);
    const optionImageUrls = question.options.map((opt) =>
      (opt.imageUrl || "").trim(),
    );
    const optionImageIds = question.options.map((opt) =>
      (opt.imageId || "").trim(),
    );
    const matchingAnswerStrings = matchingPairs.map(
    ...
    switch (typeName) {
      ...
      case "match_the_phrase":
        typeObj = {
          ...
          slots: Array.from(
            {
              length: Math.max(0, question.question.split(/_{3,}/).length - 1),
            },
            (_, index) => `blank${index + 1}`,
          ),
          options,
            ...
            Array.from(
              {
                length: Math.max(
                  0,
                  question.question.split(/_{3,}/).length - 1,
                ),
              },
              (_, index) => [
                `blank${index + 1}`,
      ...
      case "matching":
        typeObj = {
          ...
          correctMatches: Object.fromEntries(
            matchingPairs.map((pair) => [pair.left, pair.right]),
          ),
        };
        break;
      case "ranking":
    ...
    return {
      ...
      options: typeName === "matching" ? [] : options,
      correctAnswers:
        typeName === "short_answer" ||
        typeName === "fill_in_blank" ||
        typeName === "match_the_phrase" ||
        typeName === "calendar"
          ? acceptedTextAnswers
          : typeName === "matching"
  ...
  const validateQuiz = (): boolean => {
    ...
    for (let i = 0; i < slides.length; i++) {
      ...
      const hasContent =
        slide.title.trim() || slide.content.trim() || slide.imageUrl.trim();
      if (!hasContent) {
        showSnackbar(
          `Slide ${i + 1} is empty. Add title/content/image or remove it.`,
          "error",
        );
        return false;
      }
    ...
    for (let i = 0; i < questions.length; i++) {
      ...
      if (q.type === "short_answer") {
        if (q.acceptedAnswers.length < 1) {
          showSnackbar(
            `Question ${i + 1} needs at least one accepted answer`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
      ...
      if (q.type === "calendar") {
        if (q.acceptedAnswers.length < 1) {
          showSnackbar(
            `Question ${i + 1} needs at least one correct date in YYYY-MM-DD format`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
        ...
        if (!allValidDates) {
          showSnackbar(
            `Question ${i + 1} has invalid date format. Use YYYY-MM-DD.`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
      ...
      if (q.type === "numerical") {
        ...
        if (!value || !Number.isFinite(Number(value))) {
          showSnackbar(
            `Question ${i + 1} needs a valid numerical answer`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
      ...
      if (q.type === "fill_in_blank") {
        ...
        if (blankCount < 1) {
          showSnackbar(
            `Question ${i + 1} must include at least one blank (____)`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
        ...
        const allBlankAnswersFilled = Array.from({ length: blankCount }).every(
          (_, blankIndex) => {
        ...
        if (!allBlankAnswersFilled) {
          showSnackbar(
            `Question ${i + 1} must have at least one answer for each blank`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
      ...
      if (q.type === "match_the_phrase") {
        ...
        if (blankCount < 1) {
          showSnackbar(
            `Question ${i + 1} needs at least one blank (___)`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
        ...
        if (
          q.acceptedAnswers.length < blankCount ||
          q.acceptedAnswers.slice(0, blankCount).some((ans) => !ans.trim())
        ) {
          showSnackbar(
            `Question ${i + 1} needs an answer for each blank`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
        ...
        if (q.options.length < blankCount) {
          showSnackbar(
            `Question ${i + 1} needs a word bank for the blanks`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
        ...
        if (q.options.some((opt) => !hasOptionContent(opt))) {
          showSnackbar(
            `All words in the word bank for Question ${i + 1} must have text or an image`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
      ...
      if (q.type === "matching") {
        const pairs = q.matchingPairs.map((pair) => ({
          ...
          hasLeftContent: hasMatchingSideContent(
            pair.left,
            pair.leftImageFile,
            pair.leftImageUrl,
            pair.leftImageId,
          ),
          hasRightContent: hasMatchingSideContent(
            pair.right,
            pair.rightImageFile,
            pair.rightImageUrl,
            pair.rightImageId,
        ...
        if (pairs.length < 2) {
          showSnackbar(
            `Question ${i + 1} needs at least two matching pairs`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
        ...
        if (
          pairs.some((pair) => !pair.hasLeftContent || !pair.hasRightContent)
        ) {
          showSnackbar(
            `All matching pairs for Question ${i + 1} must have text or an image on each side`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
        ...
        const leftValues = pairs
          .map((pair) => pair.left.toLowerCase())
          .filter(Boolean);
        const rightValues = pairs
          .map((pair) => pair.right.toLowerCase())
          .filter(Boolean);
        if (
          ...
          showSnackbar(
            `Question ${i + 1} cannot repeat matching items`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
      ...
      if (options.length < 1) {
        showSnackbar(
          `Question ${i + 1} needs at least one option/item`,
          "error",
        );
        const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
        setCurrentPage(page);
      ...
      if (!allOptionsFilled) {
        showSnackbar(
          `All options for Question ${i + 1} must have text or an image`,
          "error",
        );
        const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
        setCurrentPage(page);
      ...
      if (q.type === "ranking" || q.type === "ordering") {
        if (q.options.length < 2) {
          showSnackbar(
            `Question ${i + 1} must have at least two items`,
            "error",
          );
          const page = Math.ceil((i + 1) / QUESTIONS_PER_PAGE);
          setCurrentPage(page);
  ...
  const handleSubmit = async () => {
    ...
    const questionItemsForUpload = quizItems
      .filter(
        (item): item is QuizItem & { question: QuizQuestion } =>
    ...
    const uploadQuestionImage = async (
      ...
      if (!response.ok) {
        throw new Error(
          `Question image upload failed for question ${questionIndex + 1}`,
        );
      }
    ...
    const uploadOptionImage = async (
      ...
      if (!response.ok) {
        throw new Error(
          `Option image upload failed for question ${questionIndex + 1}, option ${optionIndex + 1}`,
        );
      }
    ...
    try {
      ...
      if (response.ok) {
        ...
        const hasImageUploads = orderedQuestionForms.some(
          (q) =>
            Boolean(q.imageFile) ||
            q.options.some((opt) => Boolean(opt.imageFile)),
        );
        ...
        if (hasImageUploads && quizId) {
          ...
          for (
            let qIndex = 0;
            qIndex < orderedQuestionForms.length;
            qIndex += 1
          ) {
            ...
            for (let oIndex = 0; oIndex < q.options.length; oIndex += 1) {
              ...
              const uploaded = await uploadOptionImage(
                quizId,
                qIndex,
                oIndex,
                opt.imageFile,
              ...
              if (uploaded.imageUrl) {
                if (!questionItemsForUpload[qIndex].optionImageUrls) {
                  questionItemsForUpload[qIndex].optionImageUrls =
                    q.options.map(() => "");
                }
                questionItemsForUpload[qIndex].optionImageUrls![oIndex] =
                  uploaded.imageUrl;
              }
              ...
              if (uploaded.imageId) {
                ...
                questionItemsForUpload[qIndex].optionImageIds![oIndex] =
                  uploaded.imageId;
              }
            }
          }
          ...
          showSnackbar(
            `Quiz created successfully! Uploaded ${uploadCount} image${uploadCount === 1 ? "" : "s"}.`,
            "success",
          );
        } else if (hasImageUploads && !quizId) {
          showSnackbar(
            "Quiz created, but image upload was skipped (missing quiz id in response).",
            "error",
          );
        } else {
        ...
        showSnackbar(
          `Failed to create quiz: ${errorData.error || "Unknown error"}`,
          "error",
        );
      }
    } catch (error) {
  ...
  const renderQuestionEditor = (
    ...
    <SortableQuestionCard
      ...
      onDeleteOption={(optionIndex) =>
        deleteOption(item.questionIndex, optionIndex)
      }
      onAddAcceptedAnswer={() => addAcceptedAnswer(item.questionIndex)}
      onRemoveAcceptedAnswer={(answerIndex) =>
        removeAcceptedAnswer(item.questionIndex, answerIndex)
      }
      onAddMatchingPair={() => addMatchingPair(item.questionIndex)}
      onRemoveMatchingPair={(pairIndex) =>
        removeMatchingPair(item.questionIndex, pairIndex)
      }
  ...
  return (
    <div className={styles.container}>
      ...
      <QuizPreviewDialog
        ...
        getMatchingPreviewImageSrc={(file, url) =>
