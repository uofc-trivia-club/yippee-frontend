...
const syncLobbyTimelineState = (store: any, data: MessageResponse) => {
  ...
  if ("questionAnalytics" in lobby) {
    store.dispatch(
      gameActions.setQuestionAnalytics(lobby.questionAnalytics ?? undefined),
    );
  }
  ...
  if (lobby.currentItem) {
    ...
    if (lobby.currentItem.kind === "question" && lobby.currentItem.question) {
      store.dispatch(
        gameActions.setCurrentQuestion(lobby.currentItem.question),
      );
      if (typeof lobby.currentQuestionIndex === "number") {
        store.dispatch(
          gameActions.setCurrentQuestionIndex(lobby.currentQuestionIndex),
        );
      }
    } else {
    ...
    if (websocketActions.connect.match(action)) {
      ...
      if (!socket || socket.readyState !== WebSocket.OPEN) {
            ...
            switch (data.messageToClient) {
              case "Lobby created":
                if (data.lobby?.roomCode) {
                  ...
                  if (typeof data.lobby.quizMeta?.questionCount === "number") {
                    store.dispatch(
                      gameActions.setQuestionCount(
                        data.lobby.quizMeta.questionCount,
                      ),
                    );
                  }
                  ...
                  store.dispatch(
                    gameActions.setFinalQuestionLeaderboard(false),
                  );
                  if (data.clientsInLobby) {
                    store.dispatch(
                      gameActions.upsertClientsInLobby(data.clientsInLobby),
                    );
                  }
              ...
              case "Joined lobby":
                if (data.lobby?.roomCode) {
                  store.dispatch(gameActions.setRoomCode(data.lobby.roomCode));
                  store.dispatch(
                    gameActions.setGameStatus(data.lobby.status || "Waiting"),
                  );
                  if (typeof data.lobby.quizMeta?.questionCount === "number") {
                    store.dispatch(
                      gameActions.setQuestionCount(
              ...
              case "Viewing the leaderboard":
                ...
                if (data.lobby?.questionAnalytics) {
                  store.dispatch(
                    gameActions.setQuestionAnalytics(
                      data.lobby.questionAnalytics,
                    ),
                  );
                }
              ...
              case "Next question":
                ...
                if (data.lobby?.currentQuestion) {
                  store.dispatch(
                    gameActions.setCurrentQuestion(data.lobby.currentQuestion),
                  );
                }
                if (typeof data.lobby?.currentQuestionIndex === "number") {
                  store.dispatch(
                    gameActions.setCurrentQuestionIndex(
                      data.lobby.currentQuestionIndex,
                    ),
                  );
                }
                syncLobbyTimelineState(store, data);
        ...
        socket.onerror = (err) => {
          ...
          store.dispatch(
            websocketActions.connectionError("WebSocket error occurred"),
          );
          socket = null;
        };
      }
