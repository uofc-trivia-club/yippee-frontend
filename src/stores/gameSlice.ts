...
const gameSlice = createSlice({
  ...
  reducers: {
    ...
    setLastSubmittedQuestion: (
      state,
      action: PayloadAction<QuizQuestion | undefined>,
    ) => {
      state.lastSubmittedQuestion = action.payload;
    },
    setQuestionAnalytics: (
      state,
      action: PayloadAction<GameState["questionAnalytics"]>,
    ) => {
    ...
    upsertClientsInLobby: (state, action: PayloadAction<User[]>) => {
      ...
      const currentAnonymousRef = normalize(
        (state.user as User & { anonymousRef?: string }).anonymousRef,
      );
      ...
      if (matchedUser) {
        console.log("[Lobby sync] matched current user", {
          ...
          currentAnonymousRef: (state.user as User & { anonymousRef?: string })
            .anonymousRef,
