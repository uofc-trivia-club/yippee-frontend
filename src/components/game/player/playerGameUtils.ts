...
const sortNormalized = (values: string[]) =>
  [...values].map(normalizeText).sort();
...
export const isAnswerCorrectFor = (
  ...
  switch (type.name) {
    case "multiple_choice":
      return submitted[0]
        ? normalizeText(submitted[0]) ===
            normalizeText(
              (type as any).correctAnswer || question.correctAnswers?.[0] || "",
            )
        : false;

    case "multi_select":
    case "image_based": {
      const accepted = (question.correctAnswers ||
        (type as any).correctAnswers ||
        []) as string[];
    ...
    case "short_answer": {
      ...
      return accepted.some(
        (answer: string) =>
          answer === submittedNormalized ||
          isFuzzyMatch(submittedNormalized, answer),
      );
    }
    ...
    case "fill_in_blank": {
      const blankCount = Math.max(
        1,
        question.question.split("____").length - 1,
      );
      const groupedAccepted = (question.correctAnswers ||
        (type as any).correctAnswers ||
        []) as string[];
      const submittedTrimmed = submitted
        .slice(0, blankCount)
        .map((s) => normalizeText(s || ""));
      if (
        submittedTrimmed.length < blankCount ||
        submittedTrimmed.some((s) => !s)
      )
        return false;
      ...
      return submittedTrimmed.every((value, index) => {
        ...
        if (!acceptedValues.length) {
          ...
          return fallback
            ? fallback === value || isFuzzyMatch(value, fallback)
            : false;
        }
        return acceptedValues.some(
          (accepted) => accepted === value || isFuzzyMatch(value, accepted),
        );
      });
    }
    ...
    case "match_the_phrase": {
      ...
      const fromMap =
        typeAny.correctAssign &&
        typeof typeAny.correctAssign === "object" &&
        !Array.isArray(typeAny.correctAssign)
          ? Object.entries(typeAny.correctAssign as Record<string, string>).map(
      ...
      const fromOrdered =
        slotIds.length > 0 && orderedAnswers.length > 0
          ? slotIds
              .map((slotId, index) => {
      ...
      const submittedMap =
        Object.keys(submittedMapFromPairs).length > 0
      ...
      if (Object.keys(submittedMap).length !== expectedKeys.length)
        return false;
      return expectedKeys.every(
        (key) => submittedMap[key] === expectedMap[key],
      );
    }
    ...
    case "matching": {
      const typeAny = type as any;
      const fromMap =
        typeAny.correctMatches && typeof typeAny.correctMatches === "object"
          ? Object.entries(
              typeAny.correctMatches as Record<string, string>,
            ).map(([left, right]) => `${left}:${right}`)
          : [];
      const fromPairs = Array.isArray(typeAny.pairs)
        ? (
            typeAny.pairs as Array<{
              left?: string;
              right?: string;
              leftItem?: string;
              rightItem?: string;
            ...
            .filter(Boolean)
        : [];
      const legacy = Array.isArray(question.correctAnswers)
        ? question.correctAnswers
        : [];
      const correct =
        fromMap.length > 0
    ...
    case "ranking":
    case "ordering": {
      const correctOrder = ((type as any).correctOrder ||
        question.correctAnswers ||
        []) as string[];
      return (
        ...
        submitted.every(
          (value, index) =>
            normalizeText(value) === normalizeText(correctOrder[index] || ""),
        )
      );
    }

    case "calendar": {
      const correctDates = (question.correctAnswers ||
        (type as any).correctAnswers ||
        []) as string[];
      if (submitted.length === 0 || correctDates.length === 0) return false;
      const submittedNormalized = new Set(
        submitted.map((d) => d.trim()).sort(),
      );
      const correctNormalized = new Set(
        correctDates.map((d) => d.trim()).sort(),
      );
      if (submittedNormalized.size !== correctNormalized.size) return false;
      return Array.from(submittedNormalized).every((date) =>
        correctNormalized.has(date),
      );
    }
