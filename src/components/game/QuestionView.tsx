...
export default function QuestionView({
  ...
  const getQuestionTypeTitle = (typeName?: string) => {
    switch (typeName) {
      ...
      case "match_the_phrase":
        return "Match-the-phrase question";
      case "matching":
  ...
  const optionTileSx = (isCorrect: boolean) =>
    ({
      ...
      border: "1px solid",
      borderColor:
        isCorrect && displayCorrectAnswers
  ...
  const matchPhraseCorrectAssign = useMemo(() => {
    ...
    if (
      fromType &&
      typeof fromType === "object" &&
      !Array.isArray(fromType) &&
      Object.keys(fromType).length > 0
    ) {
    ...
    const orderedAnswers = (
      Array.isArray(typeAny?.correct)
        ? typeAny.correct
        : Array.isArray(typeAny?.correctAnswers)
          ? typeAny.correctAnswers
    ...
    const fromQuestion: string[] = Array.isArray(q?.correctAnswers)
  ...
  const answerBreakdown = useMemo(() => {
    ...
    if (t?.name === "match_the_phrase") {
      ...
      const fallbackSlots = Array.from(
        { length: Object.keys(matchPhraseCorrectAssign).length },
    ...
    if (questionOptions.length > 0) {
      const optionEntries = questionOptions.map((option, index) => {
        ...
        const isCorrect =
          t?.name === "multiple_choice"
            ? (t as any).correctAnswer === option
            : t?.name === "multi_select"
              ? Array.isArray((t as any).correctAnswers) &&
                (t as any).correctAnswers.includes(option)
              : t?.name === "dropdown"
                ? (t as any).correctAnswer === option
                : t?.name === "true_false"
                  ? (t as any).correctAnswer === option
  ...
  const renderOptionsView = () => {
    ...
    switch (t.name) {
            ...
            <Box key={index} sx={optionTileSx(isCorrect)}>
              <Stack direction="row" spacing={1.25} alignItems="center">
                <Chip
                  ...
                  size="small"
                  color={
                    displayCorrectAnswers && isCorrect ? "success" : "default"
                  }
                  variant={
                    displayCorrectAnswers && isCorrect ? "filled" : "outlined"
                  }
      ...
      case "multi_select": {
        const options = t.options || q.options || [];
        const correctAnswers = Array.isArray(t.correctAnswers)
          ? t.correctAnswers
          : [];
        return options.map((option, index) => {
      ...
      case "short_answer":
      case "fill_in_blank": {
        const acceptedAnswers = Array.isArray(t.correctAnswers)
          ? t.correctAnswers
          : [];
      ...
      case "numerical": {
        const numericalAnswer =
          ...
          ((t as any).correctAnswers && Array.isArray((t as any).correctAnswers)
            ? (t as any).correctAnswers[0]
            : undefined) ??
      ...
      case "calendar": {
        const acceptedDates = ((t as any).correctAnswers ||
          q?.correctAnswers ||
          []) as string[];
        return displayCorrectAnswers ? (
          <Stack spacing={1}>
      ...
      case "matching": {
        const pairs = ((t as any).pairs || []) as Array<{
          left?: string;
          right?: string;
          leftItem?: string;
          rightItem?: string;
        ...
        const left: string[] =
          pairs.length > 0
            ...
            : Array.isArray((t as any).leftItems)
              ? (t as any).leftItems
              : [];
        const right: string[] = Array.isArray(shuffledMatchingRightItems)
          ? shuffledMatchingRightItems
          : [];
        const correctMatches = ((t as any).correctMatches || {}) as Record<
          string,
          string
        >;
        const revealedPairs: Array<{ left: string; right: string }> =
          pairs.length > 0
            ? pairs
                .map((p) => ({
                  left: p.left || p.leftItem || "",
                  right: p.right || p.rightItem || "",
                }))
            ...
            : left
                .map((leftItem, index) => ({
                  left: leftItem,
                  right:
                    correctMatches[leftItem] ||
                    ((t as any).rightItems || [])[index] ||
                    "",
                }))
                .filter((pair) => Boolean(pair.left) && Boolean(pair.right));
      ...
      case "ranking":
      case "ordering": {
        const allItems = Array.isArray((t as any).items)
          ? (t as any).items
          : Array.isArray(q?.options)
            ? q.options
            : [];
        const correctOrder =
          Array.isArray((t as any).correctOrder) &&
          (t as any).correctOrder.length > 0
        ...
        const hasCompleteCorrectOrder =
          correctOrder.length > 0 && correctOrder.length === allItems.length;
        const displayItems =
          displayCorrectAnswers && hasCompleteCorrectOrder
            ? correctOrder
            : shuffledItems;
            ...
            <Stack spacing={1}>
              {displayItems.map((item: string, displayIdx: number) => {
                ...
                const imageUrl = resolveMediaUrl(
                  q?.optionImageUrls?.[
                    originalIdx >= 0 ? originalIdx : displayIdx
                  ],
                );
                return (
                  <Box
                    ...
                    sx={{
                      ...optionTileSx(true),
                      borderColor: displayCorrectAnswers
                        ? theme.palette.success.main
                        : theme.palette.divider,
